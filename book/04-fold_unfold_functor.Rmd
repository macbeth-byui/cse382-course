...
The `fold` and `unfold` are part of the common functor design patterns.  A `fold` will convert from a list to a single value whereas an `unfold` will convert from a single value to a list. 
