...
:::: {.formulabox data-latex=""}
$map(\lambda_g \circ \lambda_h) = map(\lambda_g) \circ map(\lambda_h)$. 
::::
$\nonumber$
...
In the problem set, you will implement both sides of the composition $map(\lambda_g \circ \lambda_h) = map(\lambda_g) \circ map(\lambda_h)$ to see if it holds true using a simple list of numbers.  The left hand side calls `map` once and the right hand side calls `map` twice.  The left hand side will need to create a new lambda function that composes both lambda functions.  In that new lambda function, you should use $\lambda_g$ and $\lambda_h$.

The use of composition here is a preview for the topic of chaining which we see in the near future.
